...
It has no return type and takes string, string, string, string, long, long, string, int, int, int, string, string as parameters
*/
Admin::Admin(string aid,string f_name,string l_name,string bg,long aadhaar,long phno,string gender,int d,int m,int y,string address,string password) : Person(f_name,l_name,bg,aadhaar,phno,gender,d,m,y,address,password)
{
	id = aid;
}
...
*/
void Admin :: get_A_Choice(string id)
{
...
{
	cout<<endl<<"\n--------------------------------------------------------------------------------------------------------------------------\n"<<endl;
	cout<<"\t\t\t\t\tSLECT AN OPTION TO PROCEED : "<<endl<<endl;
...
bool Admin::credential_validation(string id,string pwd)
{
	int i;
	bool flg = false;
	...
	string con,temp;
	ifstream in;
	in.open("text_files/admin_credential.txt");
	if(!in.is_open())
	{
		cout<<"\nCannot open file...!!\n";
		exit(0);
