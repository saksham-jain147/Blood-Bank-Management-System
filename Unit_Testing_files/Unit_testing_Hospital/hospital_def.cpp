...
/* 
Funciton definition to get user name
...
It's return type is string and it does not take any parameter
*/
string Hospital :: getPassword()
{
     return password;
}

void Hospital :: setPassword(string password)
{
    this->password=password;
}

//defining function to validate hospital
bool Hospital::credential_validation(string id,string pwd)
{
	int i;
	bool flg = false;
	...
	string con,temp;
	ifstream in;
	in.open("text_files/hospital_credential.txt");
	if(!in.is_open())
	{
		cout<<"\nCannot open file...!!\n";
		exit(0);
...
/* 
Funciton definition to validate name
...
string hname_validation()
{
	...
	if(name == "q")
	{
		...
		return "NULL";		//Baad mai change kar denge according to program
	}
     	else
     	{
		...
		{
			cout<<endl<<"-------------------------------------------------"<<endl;
			cout<<endl<<"\tInput Limit Exceeded !!!\n\t";
			cout<<endl<<"\nPress q to Enter Main Menu...."<<endl;
			cout<<endl<<"-------------------------------------------------"<<endl;
		...
		{
			cout<<endl<<"-------------------------------------------------"<<endl;
			cout<<endl<<"\tHospital Name is too short !!!\n\t";
			cout<<endl<<"\nPress q to Enter Main Menu...."<<endl;
			cout<<endl<<"-------------------------------------------------"<<endl;
...
string haddress_validation()
{
	...
	for(int i=0;i<=address.length();i++)
	{
		...
		else
		{
			...
			address = haddress_validation();			//Recursive Function
		}
	}
...
long hphonenumber_validation()
{
	...
	{
		cout<<"Entered to Main Menu"<<endl;
		return 0;	//Baad mai change kar denge according to program
	}
	else
	{
		/*while(ph != 0)
		...
		else
		{
			...
			phonenumber = hphonenumber_validation();
		}
		return phonenumber;
	}
}
         ...
         switch(choice)
         {
              ...
              case H_MAKEORDER:   // Make order for blood bags
              {
                   ...
                   if(h->IsBloodgroupValid(record,bg))
                   {
                       ...
                       /*ifstream in;
                       in.open("text_files/orderhistory.txt");
                       h->update_orderhistory(in,id,bg,quantity);*/
              ...
              case H_UPDATE:  //editing personal details
              {
                   ...
                   do
                   {
                       ...
                       switch(ch)
                       {
                            ...
                            {
                                 string new_address;
                                 new_address=haddress_validation();
                                 if(new_address!="NULL")
                                 {
		                         in.open("text_files/Hospital_details.txt");
		                         record=h->Load_data_into_list(in);
...
It's void return type and does not take any parameter
*/
void Hospital :: hospital_accept()
{	
	//Calling functions for validation
	
	name = name_validation();
	phone_number = phonenumber_validation();
...
It's return type is string and it does not take any parameter
*/
string Hospital :: name_validation()
{
	string name;
	int count = 0;
...
It's return type is long and it does not take any parameter
*/
long Hospital :: phonenumber_validation()
{
	long int phonenumber, ph;
	int digit = 0;
...
It's return type is string and it does not take any parameter
*/
string Hospital :: address_validation()
{
	string address;
	int count=0;
...
{
    //cout<<"\nHospital ID::\t  "<<get_Latest_Hospital_Id();
    cout<<"\nName ::\t\t  "<<name;
    cout<<"\nAddress ::\t  "<<address;
    cout<<"\nPhone No. ::\t  "<<phone_number;
}
